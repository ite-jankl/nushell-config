...
def bom [] {
    let hex_signature = (open $in --raw | bytes at 0..4 | encode hex)
    { 
        file: $in,
        bom: ($boms_lookup | where { |it| $hex_signature | str starts-with $it.hex } | first | default { label: 'None/Unknown' } | get label)
    }
}
